(function () {
  ...
  var metricDefinitions = [
    ...
    {key:"distribution",label:"配息",direction:"high",hint:"最新年化配息率｜高不等於佳",format:function (v) { return Number(v).toFixed(2) + "%"; }},
  ...
  function metricScores(fund, item) {
    ...
    return metricDefinitions.map(function (definition) {
      ...
      var score = (item.metrics[definition.key] - min) / (max - min);
      return definition.direction === "low" ? 1 - score : score;
    });
  }
  ...
  function radarChart(fund, peer) {
    ...
    var labels = metricDefinitions.map(function (definition, index) {
      return '<div class="radarAxis a' + (index + 1) + '"><b>' + definition.label + '</b><span class="fidelityStatus">' + definition.format(fund.metrics[definition.key]) + '</span><span class="peerStatus ok">' + definition.format(peer.metrics[definition.key]) + '</span><small>' + definition.hint + '</small></div>';
    }).join("");
    ...
    return '<div class="radarLegend"><span><i class="fidelityKey"></i>' + cleanName(fund.name) + '</span><span><i class="peerKey"></i>' + peer.name + '</span></div>' +
      ...
      '<p class="radarReading"><b>讀圖：</b>圖形依同類基金池的 Min–Max 相對刻度計算；績效與 Sharpe 越外越高，風險與費用越外代表越低。配息較高不代表總報酬較佳，請以原始值為準。</p>' +
  ...
  function renderCompare() {
    ...
    return '<div class="page">' + title("COMPETITOR INTELLIGENCE", "競品決策室", "先確認可比性，再談績效；沒有同日口徑，就不下勝負結論。") +
      ...
      '<div class="compareGrid"><section class="card radarCard"><div class="cardHead"><div><small>FIVE-METRIC RADAR</small><h2>五指標同類比較</h2></div><span>原始數字｜非主觀評分</span></div>' + radarChart(fund, peer) + '<p class="method">雷達圖只做同類池相對位置視覺化，不是星等，也不代表投資建議。風險採年化標準差；費用率採最高經理費；累積級別無配息紀錄時列 0.00%。</p></section>' +
